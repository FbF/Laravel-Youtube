...
class Youtube
{
    ...
    public function createPlaylist($data, $youtubeVideoId)
    {
        ...
        try {
            ...
            $resourceId->setKind(config('youtube.playlist_item.kind'));
     ...
     * @throws \Exception
     */
    public function deleteVideosFromPlayList($itemId)
    {
        try {
            //todo: check if there is no playlist?
            //$playlist = $this->searchForPlayList();
    ...
    public function searchForPlayList()
    {
        ...
        if (!array_key_exists('modelData', $playlists)
            and !array_key_exists('items', $playlists['modelData'])) {
            $newPlaylists = $playlists['items'];
        } else {
            $newPlaylists = $playlists['modelData']['items'];
        }
